...
export default function EnhancedLibrary(){
  ...
  useEffect(()=>{
    const cached = sCacheGet("md:enhanced");
    if (cached) { setMd(cached); return; }
    fetch("/scenarios/ENHANCED_PATIENT_SCENARIOS.md").then(r=>r.text()).then((t)=>{ setMd(t); sCacheSet("md:enhanced", t); }).catch(()=>setMd("Failed to load enhanced scenarios."));
  },[]);
  const sections = parseMarkdownToSections(md);
  return (
    <div className="space-y-4">
      <div className="font-medium text-lg">Enhanced Patient Scenarios</div>
      {sections.map((s, idx) => (
