...
export default function ComparePage() {
  ...
  if (!left || !right) {
    return <div className="text-sm">Select two patients from Dashboard to compare.</div>;
  }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      {[left, right].map((p) => (
        <div key={p.id} className="card">
          ...
          <div className="mt-4">
            <SimpleLineChart labels={p.vitals.dates} series={[{ name: "WBC", data: p.vitals.wbc }, { name: "Platelets", data: p.vitals.platelets }]} />
          </div>
        </div>
      ))}
    </div>
